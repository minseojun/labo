...
import { MOCK_LAB, DAYS } from '../mockData'
import { fmtDate } from '../utils'

export default function HomeTab({ user, schedules, supplies, todos, setTodos, notices, setActiveTab }) {
    ...
    <div>
      <div className="page-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div className="page-title">안녕하세요, {user.name.slice(1)}님 👋</div>
            <div className="page-subtitle">{dateStr}</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 2 }}>{MOCK_LAB.name.slice(0, 10)}...</div>
      ...
      <div className="card">
        {todos.map(t => (
          <div key={t.id} className="todo-item">
            ...
            <button onClick={() => deleteTodo(t.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text2)', fontSize: 16, padding: '0 4px' }}>×</button>
          </div>
        ))}
      ...
      {notices.filter(n => n.pinned).map(n => (
        <div key={n.id} className="notice-card">
          ...
          <div style={{ fontWeight: 500, fontSize: 13, marginBottom: 4 }}>{n.body.slice(0, 60)}...</div>
