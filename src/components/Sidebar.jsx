...
export default function Sidebar({ user, labInfo, members, onClose, onUserUpdate }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // members 서브컬렉션도 업데이트
      await updateDoc(doc(db, 'labs', user.labId, 'members', user.id), {
        name: newName.trim(),
        avatar: selAvatar,
      })
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: '48px 20px 20px', background: 'var(--green)', color: '#fff', position: 'relative' }}>
          ...
          {/* 프로필 편집 버튼 */}
          {!editing && (
            <button onClick={() => { setEditing(true); setNewName(user.name); setSelAvatar(user.avatar || '🧑‍🔬') }}
              style={{
        ...
        {editing && (
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg)' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text2)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: .6 }}>
              아바타 선택
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 14 }}>
              {AVATARS.map(av => (
                <button key={av} onClick={() => setSelAvatar(av)} style={{
                  width: 40, height: 40, borderRadius: '50%', fontSize: 20,
                  border: `2px solid ${selAvatar === av ? 'var(--green)' : 'var(--border)'}`,
                  background: selAvatar === av ? 'var(--green-light)' : 'var(--card)',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setEditing(false)} style={{
                ...
                borderRadius: 8, background: 'var(--card)', cursor: 'pointer',
                fontSize: 13, fontWeight: 600, color: 'var(--text2)'
              }}>취소</button>
              <button onClick={handleSave} disabled={saving} style={{
                flex: 2, padding: '10px', border: 'none',
                borderRadius: 8, background: 'var(--green)', color: '#fff',
          ...
          <div style={{ padding: '0 20px 16px', borderBottom: '1px solid var(--border)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
              <div style={{
                ...
                padding: '4px 12px', borderRadius: 8, fontSize: 13, fontWeight: 700, letterSpacing: 1
              }}>{labInfo?.code}</div>
            ...
            {uniqueMembers.map(m => (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0' }}>
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: m.avatar ? 20 : 12, fontWeight: 700,
                  color: roleColor(m.role), flexShrink: 0
                }}>{m.avatar || m.name?.slice(-1)}</div>
                <div style={{ flex: 1 }}>
